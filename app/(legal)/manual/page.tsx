...
export const metadata: Metadata = {
  title: 'Coachpad ユーザーマニュアル',
  description: 'Coachpad の使い方ガイド。コーチ・選手向けの全機能を説明します。',
}
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="mb-10 border-b border-slate-800 pb-8">
        ...
        <h1 className="text-3xl font-bold text-white">Coachpad の使い方</h1>
        <p className="mt-3 text-slate-400">
          トレイルランニング・耐久系競技に特化したコーチングプラットフォームです。
          コーチと選手それぞれの使い方を説明します。
        </p>
      ...
      <Section id="overview" title="Coachpad とは">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <FeatureCard
            color="violet"
            title="AIトレーニングプラン"
            desc="選手の現状とレース目標を入力するだけで、ピーキング理論に基づいた日毎のメニューをAIが自動生成"
          />
          <FeatureCard
            color="orange"
            title="Strava連携"
            desc="ガーミン・スント・カロスのデータをStrava経由で自動取り込み。記録の手間ゼロ"
          />
          <FeatureCard
            color="emerald"
            title="赤ペンフィードバック"
            desc="フォーム動画に直接書き込み。どこを直すかが一目で伝わる"
          />
          <FeatureCard
            color="blue"
            title="CTL/ATL/TSB管理"
            desc="フィットネス・疲労・調子をリアルタイムで把握。過負荷・怪我を予防"
          />
        </div>
      </Section>

      {/* ログイン */}
      <Section id="login" title="ログイン・アカウント登録">
        <SubSection title="選手の登録">
          <ol className="space-y-2 text-slate-400">
            <Step n={1} text="コーチから招待リンクを受け取る（LINE・メール等）" />
            <Step n={2} text="リンクをタップして登録画面を開く" />
            <Step n={3} text="メールアドレスとパスワードを設定して登録" />
            <Step n={4} text="初回ログイン後、プロフィールを入力する" />
          </ol>
          <Note text="選手は招待リンクなしでは登録できません。必ずコーチから招待を受けてください。" />
        </SubSection>
        <SubSection title="ログイン">
          <ol className="space-y-2 text-slate-400">
            <Step n={1} text="https://caochpad.vercel.app/login にアクセス" />
            <Step n={2} text="登録済みのメールアドレスとパスワードを入力" />
            <Step n={3} text="「ログイン」ボタンをタップ" />
          </ol>
        </SubSection>
      </Section>
      ...
      <div id="coach" className="pt-4">
        <RoleHeader role="coach" label="コーチ向け機能" color="violet" />
      </div>
      ...
      <Section id="coach-athletes" title="選手管理">
        ...
        <SubSection title="選手の詳細確認">
          <p className="text-slate-400 mb-3">選手一覧から選手名をタップすると以下を確認できます。</p>
          <Table
            headers={['項目', '内容']}
            rows={[
      ...
      <Section id="coach-ai-plan" title="AIトレーニングプラン生成">
        <p className="text-slate-400 mb-4">選手のカレンダーから「AIで週間プラン作成」ボタンで開きます。選手の現状とレース目標を元にAIが日毎のメニューを自動生成します。</p>
        ...
        <SubSection title="生成後の操作">
          <ol className="space-y-2 text-slate-400">
            <Step n={1} text="AIが生成したプランと方針が表示される" />
            <Step n={2} text="各メニューのチェックボックスで保存するものを選択" />
            <Step n={3} text="「選択したメニューをカレンダーに追加」ボタンで一括保存" />
          </ol>
        </SubSection>
      </Section>

      <Section id="coach-templates" title="筋トレテンプレート管理">
        <p className="text-slate-400 mb-4">メニュー → 筋トレ → テンプレート から管理します。繰り返し使えるトレーニングプロトコルを作成・管理できます。</p>
      ...
      <Section id="coach-exercises" title="種目ライブラリ管理">
        <p className="text-slate-400 mb-4">メニュー → 筋トレ → 種目ライブラリ から管理します。ここに登録した種目がAIプランや手動メニュー作成で使用されます。</p>
        <SubSection title="AIで一括追加">
          <ol className="space-y-2 text-slate-400">
      ...
      <Section id="coach-motion" title="フォーム動画フィードバック（赤ペン）">
        <p className="text-slate-400 mb-4">メニュー → 動作分析 から選手がアップロードした動画に直接書き込みができます。</p>
        <ol className="space-y-2 text-slate-400">
      ...
      <Section id="coach-ai-profile" title="AIプロフィール設定">
        <p className="text-slate-400 mb-4">設定 → AIプロフィール からAIがプランを生成する際に参照するコーチングスタイルを設定します。ここを充実させるほどAIの提案がコーチの意図に合ったものになります。</p>
        <Table
          headers={['項目', '内容例']}
          rows={[
            ['コーチング哲学', '「選手の自律性を重視し...」'],
      ...
      <div id="athlete" className="pt-4">
        <RoleHeader role="athlete" label="選手向け機能" color="emerald" />
      </div>

      <Section id="athlete-workout" title="ワークアウト記録">
        <p className="text-slate-400 mb-4">Strava連携を設定している場合は自動で取り込まれるため、手動入力は不要です。手動記録は カレンダー → 「記録する」 から行います。</p>
        <Table
          headers={['項目', '内容']}
          rows={[
        ...
        <SubSection title="ワークアウト達成率">
          <p className="text-slate-400 mb-3">計画メニューがある場合、実績と比較して達成率が自動計算されます。</p>
          <div className="flex flex-wrap gap-2">
      ...
      <Section id="athlete-strength" title="筋トレ実施報告">
        <p className="text-slate-400 mb-4">コーチから筋トレが割り当てられたら通知が届きます。カレンダーの筋トレメニューをタップして実施報告を行います。</p>
        <ol className="space-y-2 text-slate-400">
      ...
      <Section id="athlete-wellness" title="ウェルネス記録">
        <p className="text-slate-400 mb-4">メニュー → ウェルネス から毎日の体調を記録します。コーチのAIプラン生成にも活用されます。</p>
        <Table
          headers={['項目', 'スケール']}
          rows={[
            ['睡眠時間', '時間（数値入力）'],
      ...
      <Section id="athlete-motion" title="フォーム動画アップロード">
        ...
        <ol className="space-y-2 text-slate-400">
          ...
          <Step n={3} text="動画ファイルを選択" />
          <Step n={4} text="「アップロード」ボタンをタップ" />
        </ol>
        <Note text="コーチがフィードバックを追加すると通知が届きます。" />
      </Section>

      <Section id="athlete-strava" title="Strava連携">
        <p className="text-slate-400 mb-4">設定 → Strava連携 からガーミン・スント・カロスなどのGPSウォッチのデータをStrava経由で自動取り込みできます。</p>
        <SubSection title="初回設定手順">
          <ol className="space-y-2 text-slate-400">
            <Step n={1} text="設定画面の「Stravaに接続」ボタンをタップ" />
            <Step n={2} text="Stravaのログイン画面が開く" />
            <Step n={3} text="Stravaのメールアドレス・パスワードでログイン" />
            <Step n={4} text="「許可する」をタップ" />
            <Step n={5} text="自動でCoachpadに戻り「接続済」と表示されれば完了" />
          </ol>
        </SubSection>
        <SubSection title="連携後の動作">
          <ul className="space-y-1.5 text-slate-400">
            <Li text="Stravaにアクティビティが記録されると自動でカレンダーに反映" />
            <Li text="CTL/ATL/TSBも自動更新" />
            <Li text="過去のデータは「過去30日を同期」ボタンで手動取り込み可" />
          </ul>
        </SubSection>
      </Section>
      ...
      <div id="settings" className="pt-4">
        <RoleHeader role="common" label="共通設定" color="slate" />
      </div>
      ...
      <div className="space-y-3">
        <FaqItem
          q="Stravaに記録したのにカレンダーに反映されない"
          a="通常は数分以内に自動反映されます。反映されない場合は「設定 → Strava連携 → 過去30日を同期」ボタンで手動取り込みしてください。"
        />
        <FaqItem
          q="パスワードを忘れた"
          a="ログイン画面の「パスワードを忘れた」からリセットメールを送信できます。"
        />
        <FaqItem
          q="AIプランで筋トレメニューが生成されない"
          a="コーチが「種目ライブラリ」に種目を登録していない場合、筋トレ日は生成されません。先に種目ライブラリに種目を追加してください。"
        />
        <FaqItem
          q="心拍ゾーンが表示されない"
          a="設定 → プロフィール でLTHR（乳酸閾値心拍）を設定するとゾーンが自動計算されます。「初期テストガイド」の「20分全力走テスト」で計測できます。"
        />
        <FaqItem
          q="CTL/ATLとは何ですか"
          a="トレーニング負荷の指標です。CTL（慢性トレーニング負荷）はフィットネスの蓄積、ATL（急性トレーニング負荷）は直近の疲労、TSB（トレーニングストレスバランス）はその差で調子を表します。Strava連携またはワークアウト記録により自動更新されます。"
        />
        <FaqItem
          q="動画アップロードができない"
          a="コーチが招待済みであることが必要です。コーチが未設定の場合はアップロードできません。"
        />
      </div>
      ...
      <div className="mt-12 border-t border-slate-800 pt-8 text-center">
        <p className="text-sm text-slate-500">
          Coachpad は合同会社コアデザインが開発・運営しています。
        </p>
...
function RoleHeader({ label, color }: { role: string; label: string; color: string }) {
...
function Step({ n, text }: { n: number; text: string }) {
  return (
    <li className="flex gap-3">
      <span className="flex-shrink-0 flex h-5 w-5 items-center justify-center rounded-full bg-slate-700 text-[10px] font-bold text-slate-300 mt-0.5">
        {n}
      </span>
      <span>{text}</span>
    </li>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-700">
            {headers.map((h) => (
              <th key={h} className="pb-2 text-left text-xs font-semibold uppercase tracking-wide text-slate-400 pr-4">
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-b border-slate-800/50">
              {row.map((cell, j) => (
                <td key={j} className={`py-2 pr-4 ${j === 0 ? 'font-medium text-slate-200' : 'text-slate-400'}`}>
                  {cell}
                </td>
              ))}
            </tr>
...
function Badge({ color, label }: { color: string; label: string }) {
  ...
  return (
    <span className={`rounded-full px-3 py-1 text-xs font-medium ${colorMap[color] ?? 'bg-slate-700 text-slate-300'}`}>
      {label}
    </span>
  )
}
