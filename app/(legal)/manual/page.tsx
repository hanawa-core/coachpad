    ...
    <div className="space-y-2">
      ...
      <div className="mb-10 border-b border-slate-800 pb-8">
        <div className="mb-3 flex items-center gap-2">
          ...
          <span className="text-xs text-slate-500">v2.1 · 2026年6月</span>
        </div>
        ...
        <>
          ...
          <Section id="coach-dashboard" title="ダッシュボード">
            ...
            <ul className="mt-3 space-y-1.5 text-slate-400">
              ...
              <Li text="負荷アラート：負荷バランス（sRPE）タイプの選手でACWRが高リスク（>1.5）/低負荷（<0.8）、または持久系でTSBが極端な選手を一覧表示" />
            ...
            <SubSection title="選手の詳細確認">
              <Table
                ...
                rows={[
                  ...
                  ['負荷モデル', '自動（タイプから判定）／CTL/ATL/TSB／負荷バランス(sRPE) を手動で上書き可'],
                  ['フィジカルテスト設定', 'サッカー・テニス等の競技に必要なテストを選手ごとに割り当て'],
                  ['フィットネス指標', 'CTL・ATL・TSB（ランニング系）または 負荷バランス・ACWR（sRPE系）'],
                  ['ウェルネス推移', '体調グラフ（30日）'],
        ...
        <>
          ...
          <Section id="athlete-workout" title="ワークアウト記録">
            ...
            <Table
              ...
              rows={[
                ...
                ['時間', '分単位（全タイプ共通）'],
                ['平均・最大心拍', 'bpm'],
                ['RPE（主観強度）× 時間', '負荷バランス(sRPE)タイプ。セッション負荷(AU)を自動計算'],
                ['体重・体脂肪率・消費カロリー', 'ダイエット・運動愛好家タイプ'],
                ['TSS / CTL / ATL', 'ランニング・トレイル（持久系）タイプ（Garmin等から）'],
                ['メモ', '自由記述'],
              ]}
            />
          ...
          <Section id="athlete-wellness" title="ウェルネス記録">
            ...
            <Table
              headers={['項目', 'スケール']}
              rows={[
                ['今日のトレーニング負荷', 'RPE（0〜10）× 時間（分）。負荷バランス(sRPE)タイプのみ表示'],
                ['睡眠時間', '時間（数値入力）'],
                ['安静時心拍', 'bpm'],
                ['体重', 'kg'],
                ['睡眠の質', '1（最悪）〜 5（最高）'],
            ...
            />
            <Note text="「今日のトレーニング負荷」は練習・試合をした日にRPEと時間を入力すると、負荷バランス（ACWR）に反映されます。休養日は空欄でOK。ワークアウト記録側で入力した日はそちらが優先されます（二重計上なし）。" />
            <SubSection title="タブ">
              <div className="space-y-2">
          ...
          <Section id="athlete-load" title="負荷バランス（sRPE・ACWR）">
            <p className="text-slate-400 mb-4">
              球技・対人競技など、距離やパワーで強度を測りにくい活動向けの負荷管理です。アクティビティタイプが「競技アスリート（S&C）」「ジュニア」など持久系以外の場合に、ダッシュボードに「負荷バランス（sRPE）」カードが表示されます。ランニング・トレイル（持久系）では従来どおり CTL/ATL/TSB が表示されます。
            </p>
            <SubSection title="負荷の入力方法（どちらでもOK）">
              <ul className="space-y-1.5 text-slate-400">
                <Li text="毎日のウェルネス記録 →「今日のトレーニング負荷」でRPE（0〜10）と時間（分）を入力" />
                <Li text="または ワークアウト記録（カレンダー →「記録する」）でRPEスライダーと時間を入力" />
              </ul>
              <Note text="セッション負荷(AU) = RPE × 時間（分）で自動計算されます。同じ日に複数の練習を記録した場合は合算されます。休養日は入力不要です。" />
            </SubSection>
            <SubSection title="グラフの見方">
              <ul className="space-y-1.5 text-slate-400">
                <Li text="棒グラフ：日別の負荷（AU）" />
                <Li text="水色のライン：慢性負荷（直近28日の平均的な積み上げ＝フィットネス）" />
                <Li text="紫のライン：急性負荷（直近7日＝最近の疲労）" />
                <Li text="ACWRゲージ：急性負荷 ÷ 慢性負荷。トレーニング量の増減バランスを表します" />
              </ul>
            </SubSection>
            <SubSection title="ACWR ゾーンの目安">
              <div className="flex flex-wrap gap-2">
                <Badge color="blue" label="〜0.8 低負荷（ディトレ気味）" />
                <Badge color="emerald" label="0.8〜1.3 適正（スイートスポット）" />
                <Badge color="yellow" label="1.3〜1.5 やや高い" />
                <Badge color="red" label="1.5〜 高リスク（要調整）" />
              </div>
              <Note text="ACWRが1.5を超えると故障リスクが高まるとされます。データが28日分たまるまでは「ベースライン構築中」と表示され、ACWRは暫定扱いになります。" />
            </SubSection>
          </Section>
      ...
      <Section id="settings-activity" title="アクティビティタイプ（目的別プリセット）">
        <p className="text-slate-400 mb-4">
          選手ごとに取り組む活動を選ぶと、記録項目・表示される指標・メニュー・期分け・ダッシュボードのグラフがそのタイプに最適化されます。コーチは選手詳細ページから、選手自身は「設定 → プロフィール」から変更できます。コーチは「既定アクティビティタイプ」を設定でき、招待した新しい選手に自動適用されます。
        </p>
        <Table
          headers={['タイプ', '主な指標', '向いている人']}
          rows={[
            ['ランニング', '距離・ペース・心拍・TSS/CTL/ATL', 'レースを目指すランナー（既定）'],
            ['ジュニアスポーツ', '時間・RPE・負荷バランス(sRPE)', '成長期の選手。発育発達・多様な動作・楽しさ重視'],
            ['競技アスリート（S&C）', '時間・RPE・負荷バランス(sRPE/ACWR)', '競技の強化。ストレングス&コンディショニング'],
            ['運動愛好家', '時間・体重・体脂肪', '健康維持・運動習慣の定着'],
            ['ダイエット', '体重・体脂肪・消費カロリー', '減量・体組成改善'],
          ]}
        />
        <Note text="負荷モデルはタイプから自動で決まります。ランニング・トレイル（持久系）は CTL/ATL/TSB、それ以外は RPE×時間にもとづく 負荷バランス(sRPE/ACWR) です。コーチは選手詳細ページの「負荷モデル」から手動で切り替えることもできます（StravaとsRPEの両方のデータを持つ選手向け）。" />
      </Section>
      ...
      {role !== 'coach' && (
        <>
          <Section id="settings-races" title="ターゲットレース設定">
            <p className="text-slate-400 mb-4">設定 → ターゲットレース から目標レースを登録します。レース日から逆算したピーキング理論にもとづいて計画が組まれます。</p>
            <ol className="space-y-2 text-slate-400">
      ...
      <div className="space-y-3">
        ...
        <FaqItem q="CTL/ATLとは何ですか" a="トレーニング負荷の指標です。CTL（慢性トレーニング負荷）はフィットネスの蓄積、ATL（急性トレーニング負荷）は直近の疲労、TSB（トレーニングストレスバランス）はその差で調子を表します。Strava連携またはワークアウト記録により自動更新されます（ランニング・トレイルなど持久系タイプで表示）。" />
        <FaqItem q="ACWR・負荷バランス（sRPE）とは何ですか" a="球技・対人競技など強度を距離やパワーで測りにくい活動向けの負荷指標です。セッション負荷（RPE × 時間）を日々積み上げ、急性負荷（直近7日）÷慢性負荷（直近28日）の比をACWRと呼びます。0.8〜1.3が適正、1.5を超えると故障リスクが高いとされます。RPEと時間を入力するだけで自動計算され、ダッシュボードのゲージで確認できます。" />
        {role !== 'coach' && (
          <FaqItem q="RPEと時間はどこで入力しますか" a="2か所あります。①毎日のウェルネス記録（メニュー → ウェルネス →「記録する」）の「今日のトレーニング負荷」、②ワークアウト記録（カレンダー →「記録する」）。どちらで入力してもACWRに反映されます。同じ日に両方入れた場合はウェルネス側が優先され、二重には計上されません。" />
        )}
        <FaqItem q="負荷バランス（sRPE）が表示されない / CTLが出てほしい" a="アクティビティタイプが「競技アスリート」「ジュニア」などになっていると負荷バランス(sRPE)が、「ランニング」だとCTL/ATL/TSBが表示されます。両方のデータを持つ選手は、コーチが選手詳細ページの「負荷モデル」から手動で切り替えられます。" />
        <FaqItem q="アクティビティタイプを変更したい" a="選手自身は「設定 → プロフィール → アクティビティタイプ」から、コーチは選手詳細ページの「アクティビティタイプ」から変更できます。変更すると記録項目・指標・メニュー・グラフが切り替わります。既存の記録は保持されます。" />
